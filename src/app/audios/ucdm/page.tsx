...
const AudiosUCDMPage = () => {
    ...
    const fetchData = async (queryPage: number | string = 1, queryFilter: string) => {
        try {
            const response = await fetch(`${baseUrl}/api/audio/ucdm?page=${queryPage === null ? 1 : queryPage}&${queryFilter && `sort=${queryFilter}`}&type=ucdm`);
            ...
            {loading ?
                ...
                <div className={`grid flex-col min-h-screen initial aling-center ${cairo.className}`}>
                    <div className='flex flex-col items-center mt-12'>
                        ...
                        <Image src={"/audios-ucdm-image.jpg"} width={150} className='rounded-lg' alt='mandalaimg' height={150} />
                        <p className='text-center w-2/5 text-slate-800 font-bold mt-8'>Aquí compartimos lecturas de Un Curso de Milagros. Palabras y reflexiones para ir hacia la paz interior de la mano de estas enseñanzas...feliz viaje.</p>
                    </div>
...
export default function AudiosUCDM() {
    return (
